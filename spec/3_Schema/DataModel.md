...
**【構造と表示順序 (Structure & Ordering)】**

- **子要素リスト (`Children`):** `List<Task>` または `List<Container>`
  - このコンテナに属する子要素の実体を保持するリスト。このリストの並び順（インデックス順）が、UI（TasksやTimeline等）での表示順となる。
- **順序のフォールバック規則 (Fallback Rule):**
  インフラ層からのデータロード（Replay）時、ファイル同期のコンフリクト等により順序データから漏れている子要素を発見した場合、システムはそれらをリストの末尾に「作成日時 (`CreatedAt`) の昇順」で自動的に追加してロードしなければならない（MUST）。
