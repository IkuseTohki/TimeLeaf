...
- **論理仕様**: 背景に `BrandPrimaryBrush` を 100% 適用し、文字色は背景とのコントラストが高い色（`OnBrandPrimaryBrush` 相当）とする。
- **状態変化**:
  - **Hover**: `BrandPrimaryHoverBrush` を適用、または背景の彩度をわずかに上げる、もしくは輝度を 10% 変更する。
  - **Active**: 押し込まれた感覚を与えるため、不透明度を 10% 下げる。
- **物理実装例 (WPF)**: `PrimaryButtonStyle` を適用する。

### 1.2. セカンダリボタン (Secondary Button)

補助的なアクション（キャンセル、閉じる等）に使用する。

- **論理仕様**: 強い背景色を持たず、境界線（`OutlineBaseBrush`）または控えめな背景色で存在を示す。
- **状態変化**: Hover 時にのみ、デザインシステムで定めた `StateHoverBrush` を適用する。

### 1.3. アイコンボタン (Icon Button)

ツールバーや省スペースな場所での操作に使用する。

- **論理仕様**: 通常時は背景・境界線ともに透明。Soft Emphasis 規約に基づき、透過的な塗りつぶしで状態を表現する。
- **状態変化 (Soft Emphasis)**:
  - **Hover**: `StateHoverBrush`（グレー系統の透過背景）を表示。
  - **Selected**: `EmphasisLowBrush`（アクセントカラー透過背景）を表示し、アイコン自体の色を `EmphasisHighBrush` へ変更する。
...
- **論理仕様**:
  - **形状**: 角丸 `ControlCornerRadius` (4px)、境界線 1px。
  - **色彩**: 通常時の境界線は `OutlineBaseBrush` とし、入力エリアの背景は `SurfaceBaseBrush` に従う。
- **フォーカス状態 (Focus)**: ユーザーが入力を開始した際、境界線の色を `BrandPrimaryBrush` へ変更する。この際、150ms 程度の滑らかな遷移を伴うものとする。
...
- **論理仕様**: リスト表示時も全体のトーンを維持するため、ポップアップ部分の背景色およびホバー色を、共通のデザイン・トークンと完全に同期させる。
- **制約**: OS のデフォルトカラーによる上書きを許容せず、常に `TextPrimaryBrush` による視認性を確保する。
